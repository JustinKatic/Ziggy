...
public class BallControllerMedium : MonoBehaviour {
	...
	void Update ()
	{
		if (!started) 
		{
			if (Input.GetMouseButtonDown (0))
			{
				...
				GameManagerMedium.instance.StartGame ();
			}
		}
		...
		if (!Physics.Raycast (transform.position, Vector3.down, 1f) && !gameOver)
		{
			...
			GameManagerMedium.instance.GameOver ();
		}
	...
	void OnTriggerEnter(Collider col)
	{
		if (col.gameObject.tag == "Diamond")
		{
			...
			ScoreManagerMedium.instance.score += 2;
